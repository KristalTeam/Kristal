...
function spell:init(chara, style)
    ...
    -- The base amount of healing points
    self.base_amount = 1
    
    -- The healing multiplier of the magic stat
    self.magic_stat_multiplier = 1
...
function spell:onCast(user, target)
    ...
    local base_heal = math.ceil((user.chara:getStat("magic") * self.magic_stat_multiplier) + self.base_amount + (self.usage_bonus_multiplier * math.min(user.chara:getFlag("ultimateheals_used", 0), self.usage_bonus_limit)))
