...
function Game:enter(previous_state)
    ...
    if MOD and MOD.map then
        ...
        if previous_state == Kristal.States["DarkTransition"] then
            ...
            previous_state.fall_y = py

            local kx, ky = previous_state.kris_sprite:getScreenPos()
            print(kx, ky)
...
function Game:update(dt)
    ...
    if self.world.player and not disabled then
        ...
        if walk_x ~= 0 or walk_y ~= 0 then
            self.world.camera.x = Utils.approach(self.world.camera.x, self.world.player.x, 16 * DTMULT)
            self.world.camera.y = Utils.approach(self.world.camera.y, self.world.player.y, 16 * DTMULT)
        end
    end
...
function Game:keypressed(key)
    if self.previous_state.animation_active then return end
