...
--- Additionally, an EditorObject owns the creation of its associated game-time object.
--- This should be used rather than the legacy runtime event registry.
---@class EditorObject : Class
---@field data table
---@field height number
---@field id string?
---@field layer number
...
---@overload fun(data?: table, options?: table): EditorObject
local EditorObject = Class()

---@class EditorObjectInteraction
---@field id string
---@field cursor string?
---@field name string?

---@class EditorObjectInteractionContext
...
-- Object classes may override this with "point" when their position has no
-- bounded area, or "region" when placement should be defined by dragging.
EditorObject.placement_shape = "rectangle"
...
function EditorObject:registerPropertyGroup(id, options)
    return self.property_set:registerGroup(id, options)
end

function EditorObject:init(data, options)
    data = data or {}
    options = options or {}
...
function EditorObject:getShapeData()
    return { self.data.width, self.data.height, self.data.polygon }
end

function EditorObject:getRectData()
    return { self.data.width, self.data.height }
end

function EditorObject:getCharacterPosition(map)
...
---@return Object?
function EditorObject:createObject(map, context)
    error(ClassUtils.getClassName(self) .. ":createObject() must be overridden", 2)
end

function EditorObject:getBoundsSize()
...
function EditorObject:getSortPosition()
    local width, height = self:getBoundsSize()
    return self.x + width / 2, self.y + height
end

---@param context EditorObjectInteractionContext
function EditorObject:drawEditorSelection(context)
end

---@param x number
---@param y number
---@param context EditorObjectInteractionContext
---@return EditorObjectInteraction?
function EditorObject:getEditorInteraction(x, y, context)
    return nil
end

---@param interaction EditorObjectInteraction
---@param x number
---@param y number
---@param context EditorObjectInteractionContext
---@return boolean
function EditorObject:beginEditorInteraction(interaction, x, y, context)
    return true
end
...
---@param context EditorObjectInteractionContext
---@return boolean
function EditorObject:updateEditorInteraction(interaction, x, y, context)
    return false
end
...
---@param context EditorObjectInteractionContext
function EditorObject:endEditorInteraction(interaction, x, y, changed, context)
end

function EditorObject:getPreviewSprite(sprite)
...
function EditorObject:drawPreviewIcon(x, y, width, height, alpha)
    local texture, marker = self:getTexture()
    if not texture then return false end
    alpha = alpha or 1
...
function EditorObject:drawBounds(alpha, line_width)
