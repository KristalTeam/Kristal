...
function Loading:beginLoad()
    Kristal.log("Clear assets")
    Kristal.clearAssets(true)
    Kristal.log("Clear assets done")

    self.loading = true
    self.load_complete = false

    Kristal.log("Load assets - all")
    ...
    Kristal.loadAssets("", "mods", "", function ()
        ...
        Kristal.log("Assets.saveData")

        Assets.saveData()

        Kristal.log("ABOUT TO SET TITLE...")
        love.window.setTitle(Kristal.getDesiredWindowTitle())
        Kristal.log("WE SET THE TITLE...")
    end)
end
...
function Loading:drawScissor(image, left, top, width, height, x, y, alpha)
    ...
    Draw.scissorUntransformed(scissor_x, scissor_y, width, height)
