...
function MainMenu:drawBackground()
    ...
    if not (TARGET_MOD and self.selected_mod.preview) then
        ...
        if Kristal.supportsShaders() then
            ...
            love.graphics.setShader(Kristal.Shaders["MenuBackground"])
            Kristal.Shaders["MenuBackground"]:send("bg_sine", self.animation_sine)
            Kristal.Shaders["MenuBackground"]:send("bg_mag", 6)
            Kristal.Shaders["MenuBackground"]:send("wave_height", 240)
            Kristal.Shaders["MenuBackground"]:send("texsize",
                                                   { self.background_image_wave:getWidth(),
                                                       self.background_image_wave:getHeight() })
            Kristal.Shaders["MenuBackground"]:send("sine_mul", 1)
        end
