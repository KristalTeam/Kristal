---@class MainMenuModListAlt : StateClass
---
---@field menu MainMenu
---
---@field list ModList
...
---
---@overload fun(menu:MainMenu) : MainMenuModListAlt
local MainMenuModListAlt, super = Class(StateClass)

function MainMenuModListAlt:init(menu)
    self.menu = menu
...
function MainMenuModListAlt:registerEvents()
    self:registerEvent("enter", self.onEnter)
    self:registerEvent("leave", self.onLeave)
    self:registerEvent("keypressed", self.onKeyPressed)
    self:registerEvent("update", self.update)
    self:registerEvent("draw", function(_,...) return self:draw(...) end)
end
...
function MainMenuModListAlt:onEnter(old_state)
    self.active = true
...
function MainMenuModListAlt:onLeave(new_state)
    if self.list then
...
function MainMenuModListAlt:onKeyPressed(key, is_repeat)
    if key == "f5" then
...
function MainMenuModListAlt:update()
...
function MainMenuModListAlt:draw()
...
---@return table
function MainMenuModListAlt:getSelectedMod()
    return self.list:getSelectedMod()
end

---@return ModButton|ModCreateButton
function MainMenuModListAlt:getSelectedButton()
    return self.list:getSelected()
end

function MainMenuModListAlt:checkCompatibility()
    local mod = self:getSelectedMod()
...
function MainMenuModListAlt:reloadMods()
...
function MainMenuModListAlt:buildModListFavorited()
    ...
    for _,mod in ipairs(self.mods) do
        ...
        local button = SmallModButton(mod.name or mod.id, 62, 62, mod)
...
function MainMenuModListAlt:buildModList()
    -- Remember the last selected mod
    local last_scroll = self.list and self.list.scroll_target
    local last_selected = self.list and self.list:getSelectedId()
