...
function DarkConfigMenu:draw()
    ...
    if self.state ~= "CONTROLS" then
        Draw.print("CONFIG", 188, -12)

        if self.state == "VOLUME" then
            Draw.setColor(PALETTE["world_text_selected"])
        end
        Draw.print("Master Volume", 88, 38 + (0 * 32))
        Draw.setColor(PALETTE["world_text"])
        Draw.print("Controls", 88, 38 + (1 * 32))
        Draw.print("Simplify VFX", 88, 38 + (2 * 32))
        Draw.print("Fullscreen", 88, 38 + (3 * 32))
        Draw.print("Auto-Run", 88, 38 + (4 * 32))
        Draw.print("Return to Title", 88, 38 + (5 * 32))
        Draw.print("Back", 88, 38 + (6 * 32))

        if self.state == "VOLUME" then
            Draw.setColor(PALETTE["world_text_selected"])
        end
        Draw.print(Utils.round(Kristal.getVolume() * 100) .. "%", 348, 38 + (0 * 32))
        Draw.setColor(PALETTE["world_text"])
        Draw.print(Kristal.Config["simplifyVFX"] and "ON" or "OFF", 348, 38 + (2 * 32))
        Draw.print(Kristal.Config["fullscreen"] and "ON" or "OFF", 348, 38 + (3 * 32))
        Draw.print(Kristal.Config["autoRun"] and "ON" or "OFF", 348, 38 + (4 * 32))
        ...
        Draw.print("Function", 23, -12)
        -- Console accuracy for the Heck of it
        if not Kristal.isConsole() then
            Draw.print("Key", 243, -12)
        end
        if Input.hasGamepad() then
            Draw.print(Kristal.isConsole() and "Button" or "Gamepad", 353, -12)
        end
        ...
        for index, name in ipairs(Input.order) do
            ...
            if dualshock then
                Draw.print(name:gsub("_", " "):upper(), 23, -4 + (29 * index))
            else
                Draw.print(name:gsub("_", " "):upper(), 23, -4 + (28 * index) + 4)
            end
            ...
            if not Kristal.isConsole() then
                ...
                if type(alias) == "table" then
                    ...
                    Draw.print(table.concat(title_cased, "+"), 243, 0 + (28 * index))
                elseif alias ~= nil then
                    Draw.print(Utils.titleCase(alias), 243, 0 + (28 * index))
                end
            end
        ...
        if dualshock then
            Draw.print("Reset to default", 23, -4 + (29 * 8))
        else
            Draw.print("Reset to default", 23, -4 + (28 * 8) + 4)
        end
        ...
        if dualshock then
            Draw.print("Finish", 23, -4 + (29 * 9))
        else
            Draw.print("Finish", 23, -4 + (28 * 9) + 4)
        end
