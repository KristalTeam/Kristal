...
function LightSaveMenu:draw()
    ...
    Draw.print(name,         self.box.x + 8,        self.box.y - 10 + 8)
    Draw.print("LV "..level, self.box.x + 210 - 34, self.box.y - 10 + 8)
    ...
    if self.state == "MAIN" then
        Draw.print("Save",   self.box.x + 30  + 8, self.box.y + 98)
        Draw.print("Return", self.box.x + 210 + 8, self.box.y + 98)

        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite, self.box.x + 10 + (self.selected_x - 1) * 180, self.box.y + 96 + 8, 0, 2, 2)
    elseif self.state == "SAVED" then
        Draw.print("File saved.", self.box.x + 30 + 8, self.box.y + 98)
    end
