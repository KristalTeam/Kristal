    ...
    elseif self.state == "INFO" then
        ...
        for i, recruit in pairs(self.recruits) do
            ...
            if i == self.selected then
                ...
                for j, value in ipairs({ "LIKE", "DISLIKE", "WISH", "?????" }) do
                    local x_scale = 1
                ...
                for j, value in ipairs({ Game:hasRecruit(recruit.id) and recruit:getLike() or "?", Game:hasRecruit(recruit.id) and recruit:getDislike() or "?", Game:hasRecruit(recruit.id) and recruit:getWish() or "?", "?????????" }) do
                    local x_scale = 1
