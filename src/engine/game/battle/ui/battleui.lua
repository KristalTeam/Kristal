...
function BattleUI:init()
    ...
    self.short_act_text_1 = DialogueText("", 30, 51, SCREEN_WIDTH - 30, SCREEN_HEIGHT - 53,
        { wrap = false, line_offset = 0 })
    self.short_act_text_2 = DialogueText("", 30, 51 + 30, SCREEN_WIDTH - 30, SCREEN_HEIGHT - 53,
        { wrap = false, line_offset = 0 })
    self.short_act_text_3 = DialogueText("", 30, 51 + 30 + 30, SCREEN_WIDTH - 30, SCREEN_HEIGHT - 53,
        { wrap = false, line_offset = 0 })
...
function BattleUI:drawActionStrip()
    ...
    love.graphics.rectangle("fill", 0, Game:getConfig("oldUIPositions") and 1 or 0, 640,
        Game:getConfig("oldUIPositions") and 3 or 2)
    -- Draw the background of the action strip
    Draw.setColor(PALETTE["action_fill"])
    love.graphics.rectangle("fill", 0, Game:getConfig("oldUIPositions") and 4 or 2, 640,
        Game:getConfig("oldUIPositions") and 33 or 35)
end
...
function BattleUI:drawState()
    if Game.battle.state == "MENUSELECT" then
        ...
        Draw.draw(self.heart_sprite, 5 + ((Game.battle.current_menu_x - 1) * 230),
            30 + ((Game.battle.current_menu_y - (page * 3)) * 30))
            ...
            if item.party then
                ...
                for index, party_id in ipairs(item.party) do
                    ...
                    if Game.battle:getPartyIndex(party_id) ~= Game.battle.current_selecting then
                        ...
                        Draw.draw(Assets.getTexture(chara:getHeadIcons() .. "/head"), text_offset + 30 + (x * 230) + ox,
                            50 + (y * 30) + oy)
        ...
        if current_item.tp and current_item.tp ~= 0 then
            ...
            love.graphics.print(
            math.floor((current_item.tp / Game:getMaxTension()) * 100) .. "% " .. Game:getConfig("tpName"), 260 + 240,
                50 + (tp_offset * 32))
        ...
        for index = page_offset + 1, math.min(page_offset + 3, #enemies) do
            ...
            if Game.battle.state == "XACTENEMYSELECT" then
                ...
                if Game.battle.selected_xaction.id == 0 then
                    love.graphics.print(enemy:getXAction(Game.battle.party[Game.battle.current_selecting]), 282,
                        50 + y_off)
                else
    ...
    elseif Game.battle.state == "PARTYSELECT" then
        ...
        for index = page_offset + 1, math.min(page_offset + 3, #Game.battle.party) do
            ...
            local percentage = Game.battle.party[index].chara:getHealth() /
            Game.battle.party[index].chara:getStat("health")
            Draw.setColor(PALETTE["action_health"])
